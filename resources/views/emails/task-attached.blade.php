...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
  ...
  <tr><td style="padding:32px;">
    <h2 style="margin:0 0 16px;color:#111;font-size:18px;">Files attached to: {{ $task->title }}</h2>
    <p style="margin:0 0 8px;color:#333;font-size:14px;">Hi {{ $recipient->name }},</p>
    <p style="margin:0 0 20px;color:#555;font-size:14px;">{{ $actor->name }} attached {{ count($attachments) }} file(s) to task "{{ $task->title }}".</p>
    ...
    <br>
    <a href="{{ $url }}" style="display:inline-block;padding:12px 28px;background:#000;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;">View Task</a>
  </td></tr>
