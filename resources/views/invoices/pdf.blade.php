...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .doc-number strong { color: #111827; font-weight: 600; }
    ...
    .meta td { width: 33.33%; padding-right: 16px; }
    ...
    .meta .m-value.overdue { color: #dc2626; }
    .meta .m-value.paid { color: #059669; }
    ...
    .totals .t-value.discount { color: #dc2626; }
    .totals .t-value.paid { color: #059669; }
    ...
    .install-caption strong { color: #111827; font-weight: 700; }
    .install-bar {
        height: 6px;
        background: #e5e7eb;
        border-radius: 999px;
    ...
    .pay-date { color: #6b7280; width: 22%; }
    .pay-method { width: 22%; color: #4b5563; }
    .pay-note {
        color: #9ca3af;
        font-style: italic;
    }
    ...
    .footer strong { color: #4b5563; font-weight: 600; }
</style>
</head>
<body>
...
<tr>
    <td>
        ...
        @if($client->contact)<div class="p-line">{{ $client->contact }}</div>@endif
        @if($client->email)<div class="p-line">{{ $client->email }}</div>@endif
        @if($client->phone)<div class="p-line">{{ $client->phone }}</div>@endif
    </td>
    <td>
        <div class="p-label">From</div>
        <div class="p-name">{{ $agency->agency_name ?? 'Madhyam' }}</div>
        @if($agency->agency_address ?? null)<div class="p-line">{{ $agency->agency_address }}</div>@endif
        @if($agency->agency_email)<div class="p-line">{{ $agency->agency_email }}</div>@endif
        @if($agency->agency_phone)<div class="p-line">{{ $agency->agency_phone }}</div>@endif
    </td>
</tr>
</table>
    ...
    <div class="section">
        ...
        <div class="install-progress">
            <div class="install-caption">
                <strong>{{ $paid }} of {{ $total }}</strong> installments paid
                &nbsp;·&nbsp; Rs. {{ number_format($per, 2) }} per installment
            </div>
    ...
    <div class="section">
        ...
        <table class="payments" cellpadding="0" cellspacing="0">
            @foreach($payments as $payment)
                <tr>
                    <td class="pay-date" style="width: 20%;">{{ \Carbon\Carbon::parse($payment->date)->format('M d, Y') }}</td>
                    <td class="pay-method" style="width: 20%;">
                        {{ ucfirst($payment->method) }}
                        @if($payment->verified)<span class="verified-tick">✓</span>@endif
                    </td>
...
<div class="footer">
    @if($isPaid)
        <strong>Thank you for your payment.</strong> This document serves as your receipt.
    @else
        <strong>Thank you for your business.</strong> Please settle by {{ $invoice->due_date->format('M d, Y') }} to avoid service disruption.
    @endif
    <br>
