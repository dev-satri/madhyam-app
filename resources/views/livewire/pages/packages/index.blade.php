    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        @endphp
            ...
            >
                <div class="flex items-start justify-between mb-3">
                    ...
                    <span
                        class="badge {{ $pkg['status'] === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}"
                ...
                <div class="flex items-center gap-4 mt-3 text-xs text-gray-500">
                    ...
                    @if ($overLimitCount > 0)
                        <span class="text-red-500"
                            ><i class="fas fa-exclamation-circle mr-1"></i>{{ $overLimitCount }} over</span
                        >
                    @endif
                </div>
                <div class="mt-4 space-y-2">
                    <div>
                        <div class="flex justify-between text-[11px] mb-0.5">
                            ...
                            <span
                                class="font-semibold {{ $avgContentPct >= 90 ? 'text-red-500' : ($avgContentPct >= 70 ? 'text-amber-500' : 'text-green-500') }}"
                                >{{ $avgContentPct }}%</span
                            >
                        </div>
                        <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div
                                class="h-full rounded-full {{ $avgContentPct >= 90 ? 'bg-red-500' : ($avgContentPct >= 70 ? 'bg-amber-500' : 'bg-green-500') }}"
                                style="width: {{ $avgContentPct }}%"
                            ></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between text-[11px] mb-0.5">
                            ...
                            <span
                                class="font-semibold {{ $avgWorkflowPct >= 90 ? 'text-red-500' : ($avgWorkflowPct >= 70 ? 'text-amber-500' : 'text-green-500') }}"
                                >{{ $avgWorkflowPct }}%</span
                            >
                        </div>
                        <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div
                                class="h-full rounded-full {{ $avgWorkflowPct >= 90 ? 'bg-red-500' : ($avgWorkflowPct >= 70 ? 'bg-amber-500' : 'bg-green-500') }}"
                                style="width: {{ $avgWorkflowPct }}%"
                            ></div>
                        </div>
                    </div>
                </div>
                <div class="flex flex-wrap gap-1.5 mt-3">
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600"
                        ><i class="fas fa-file-alt mr-0.5"></i>{{ $pkg['content_limit'] }}/mo</span
                    >
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600"
                        ><i class="fas fa-columns mr-0.5"></i>{{ $pkg['workflow_limit'] }}/mo</span
                    >
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600"
                        ><i class="fas fa-hdd mr-0.5"></i>{{ $pkg['storage_limit_mb'] }}MB</span
                    >
                    @if ($pkg['priority_support'])
                        <span class="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-700"
                            ><i class="fas fa-headset mr-0.5"></i>Priority</span
                        >
                    @endif
                </div>
                @if (!empty($pkg['included_platforms']))
                    <div class="flex flex-wrap gap-1 mt-2">
                        @foreach ($pkg['included_platforms'] as $platform)
                            <span
                                class="text-[10px] px-1.5 py-0.5 rounded-full bg-[var(--brand)]/10 text-[var(--brand)]"
        ...
        @empty
            <div class="col-span-1 sm:col-span-2 lg:col-span-4 text-center py-12">
                ...
                <button wire:click="openPkgForm" class="btn btn-primary btn-sm mt-3">
                    <i class="fas fa-plus mr-1"></i> Create First Package
                </button>
            </div>
        @endforelse
    </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <select wire:model.live="packageFilter" class="form-select text-xs py-1.5 px-3 w-36">
                        ...
                        @foreach ($packages as $pkg)
                            <option value="{{ $pkg['slug'] }}">
                                {{ $pkg['name'] }} ({{ collect($allClientsUsage)->filter(fn($c) => $c['package_slug'] === $pkg['slug'])->count() }})
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="flex bg-gray-100 rounded-lg p-0.5">
                        <button
                            wire:click="$set('clientFilter', 'all')"
                            class="px-3 py-1.5 text-xs font-semibold rounded-md transition-colors {{ $clientFilter === 'all' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                        >
                            All <span class="ml-1 text-[10px]">{{ $counts['all'] }}</span>
                        </button>
                        <button
                            wire:click="$set('clientFilter', 'over')"
                            class="px-3 py-1.5 text-xs font-semibold rounded-md transition-colors {{ $clientFilter === 'over' ? 'bg-red-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                        >
                            <i
                                class="fas fa-circle text-[6px] mr-1 {{ $counts['over'] > 0 ? 'text-red-400' : 'text-gray-400' }}"
                            ></i
                            >Over <span class="ml-1 text-[10px]">{{ $counts['over'] }}</span>
                        </button>
                        <button
                            wire:click="$set('clientFilter', 'warning')"
                            class="px-3 py-1.5 text-xs font-semibold rounded-md transition-colors {{ $clientFilter === 'warning' ? 'bg-amber-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                        >
                            <i
                                class="fas fa-circle text-[6px] mr-1 {{ $counts['warning'] > 0 ? 'text-amber-400' : 'text-gray-400' }}"
                            ></i
                            >Near <span class="ml-1 text-[10px]">{{ $counts['warning'] }}</span>
                        </button>
                        <button
                            wire:click="$set('clientFilter', 'ok')"
                            class="px-3 py-1.5 text-xs font-semibold rounded-md transition-colors {{ $clientFilter === 'ok' ? 'bg-green-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                        >
                            <i class="fas fa-circle text-[6px] mr-1 text-green-400"></i>OK
                            <span class="ml-1 text-[10px]">{{ $counts['ok'] }}</span>
                        </button>
                    </div>
                    ...
                    @if ($clientFilter !== 'all' || $packageFilter !== 'all' || $clientSearch)
                        <button
                            ...
                            class="text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5 rounded-lg hover:bg-gray-100"
                            title="Reset filters"
                        >
                    ...
                    <div class="flex bg-gray-100 rounded-lg p-0.5">
                        <button
                            ...
                            class="px-2 py-1.5 text-xs rounded-md transition-colors {{ $viewMode === 'table' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                            title="Table view"
                        >
                        ...
                        <button
                            ...
                            class="px-2 py-1.5 text-xs rounded-md transition-colors {{ $viewMode === 'grid' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}"
                            title="Grid view"
                        >
            ...
            @if ($clientFilter !== 'all' || $packageFilter !== 'all' || $clientSearch)
                <div class="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    @if ($packageFilter !== 'all')
                        <span
                            class="text-[10px] px-2 py-0.5 rounded-full bg-[var(--brand)]/10 text-[var(--brand)] font-semibold"
                        >
                            Package: {{ ucfirst($packageFilter) }}
                            <button wire:click="$set('packageFilter', 'all')" class="ml-1 hover:text-[var(--brand)]/70">
                    ...
                    @if ($clientFilter !== 'all')
                        <span
                            class="text-[10px] px-2 py-0.5 rounded-full bg-{{ $clientFilter === 'over' ? 'red' : ($clientFilter === 'warning' ? 'amber' : 'green') }}-100 text-{{ $clientFilter === 'over' ? 'red' : ($clientFilter === 'warning' ? 'amber' : 'green') }}-700 font-semibold"
                        >
                            Status: {{ ucfirst($clientFilter) }}
                            <button wire:click="$set('clientFilter', 'all')" class="ml-1">
                                <i class="fas fa-times"></i>
                            </button>
                        </span>
                    @endif
                    @if ($clientSearch)
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 font-semibold">
                            Search: "{{ $clientSearch }}"
                            <button wire:click="$set('clientSearch', '')" class="ml-1">
                                <i class="fas fa-times"></i>
                            </button>
                        </span>
                    @endif
            ...
            @if ($viewMode === 'grid')
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                    @forelse ($this->filteredClients as $client)
                        <div
                            wire:click="openClientDetail({{ $client['client_id'] }})"
                            class="group rounded-xl border border-gray-100 p-4 hover:shadow-md hover:border-[var(--brand)]/30 transition-all cursor-pointer {{ $client['is_over_limit'] ? 'border-red-200 bg-red-50/30' : ($client['needs_attention'] ? 'border-amber-200 bg-amber-50/30' : '') }}"
                        >
                            <div class="flex items-center gap-3 mb-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-[var(--brand)]/10 flex items-center justify-center text-[var(--brand)] text-sm font-bold"
                                >
                                    {{ strtoupper(substr($client['client_name'], 0, 2)) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span
                                        class="badge badge-{{ $client['package_slug'] }} text-[10px]"
                                        >{{ $client['package_name'] }}</span
                                    >
                                </div>
                                @if ($client['is_over_limit'])
                            ...
                            <div class="space-y-2">
                                <div>
                                    <div class="flex justify-between text-[11px] mb-0.5">
                                        <span class="text-gray-500">Content</span>
                                        <span
                                            class="font-semibold {{ $client['content_pct'] >= 90 ? 'text-red-500' : ($client['content_pct'] >= 70 ? 'text-amber-500' : 'text-gray-600') }}"
                                            >{{ $client['content_used'] }}/{{ $client['content_limit'] }}</span
                                        >
                                    </div>
                                ...
                                <div>
                                    <div class="flex justify-between text-[11px] mb-0.5">
                                        <span class="text-gray-500">Workflow</span>
                                        <span
                                            class="font-semibold {{ $client['workflow_pct'] >= 90 ? 'text-red-500' : ($client['workflow_pct'] >= 70 ? 'text-amber-500' : 'text-gray-600') }}"
                                            >{{ $client['workflow_used'] }}/{{ $client['workflow_limit'] }}</span
                                        >
                                    </div>
                                    <div class="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                ...
                                <div>
                                    <div class="flex justify-between text-[11px] mb-0.5">
                                        ...
                                        <span
                                            class="font-semibold {{ $client['storage_pct'] >= 90 ? 'text-red-500' : ($client['storage_pct'] >= 70 ? 'text-amber-500' : 'text-gray-600') }}"
                                            >{{ $client['storage_used'] }}/{{ $client['storage_limit'] }}MB</span
                                        >
                                    </div>
                                    <div class="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                            ...
                            <div
                                class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 text-[10px] text-gray-500"
                            >
                                <span
                                    ><i class="fas fa-check-double mr-1 text-green-500"></i
                                    >{{ $client['approvals_used'] }} approvals</span
                                >
                                <span
                                    ><i class="fas fa-file-upload mr-1 text-orange-500"></i
                                    >{{ $client['files_uploaded'] }} files</span
                                >
                            </div>
                        </div>
                    @empty
                        <div class="col-span-3 text-center py-8 text-gray-400">
                            <i class="fas fa-search text-2xl mb-2 block"></i>
                            <p class="text-sm">No clients found matching your criteria</p>
                            <button wire:click="resetFilters" class="text-xs text-[var(--brand)] mt-2 hover:underline">
                                Clear all filters
                            </button>
                        </div>
                    @endforelse
            ...
            @if (count($this->filteredClients) > 0)
                ...
                <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500">
                    <div class="flex items-center gap-4">
                        <span
                            >Showing <strong class="text-gray-700">{{ count($filtered) }}</strong> of
                            <strong class="text-gray-700">{{ $counts['all'] }}</strong> client(s)</span
                        >
                        @if ($packageFilter !== 'all')
                            <span class="text-[var(--brand)]"
                                >Filtered by: <strong>{{ ucfirst($packageFilter) }}</strong></span
                            >
                        @endif
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-gray-600"
                                >Content
                                <strong
                                    >{{ count($filtered) > 0 ? round(array_sum(array_column($filtered, 'content_pct')) / count($filtered)) : 0 }}%</strong
                                ></span
                            >
                            ...
                            <span class="text-gray-600"
                                >Workflow
                                <strong
                                    >{{ count($filtered) > 0 ? round(array_sum(array_column($filtered, 'workflow_pct')) / count($filtered)) : 0 }}%</strong
                                ></span
                            >
                            <span class="text-gray-300">·</span>
                            <span class="text-gray-600"
                                >Storage
                                <strong
                                    >{{ count($filtered) > 0 ? round(array_sum(array_column($filtered, 'storage_pct')) / count($filtered)) : 0 }}%</strong
                                ></span
                            >
                        </div>
                    </div>
            ...
            >
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            wire:model="pkgFeatures"
                            class="form-textarea w-full text-sm"
                            rows="3"
                            placeholder="8 reels per month&#10;Analytics dashboard&#10;Priority support"
                        ></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1"
                            >Included Platforms (comma-separated)</label
                        >
                        <input
                            type="text"
                            wire:model="pkgPlatforms"
                            class="form-input w-full text-sm"
            ...
            >
                ...
                >
                    <div class="flex items-center gap-3">
                        <span
                            class="badge badge-{{ $detailPackage['slug'] ?? '' }}"
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
                        ...
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p
                                class="text-2xl font-extrabold {{ $summary['avg_content_pct'] >= 80 ? 'text-amber-600' : 'text-gray-900' }}"
                            >{{ $summary['avg_content_pct'] }}%</p>
                            <p class="text-[10px] text-gray-500 mt-0.5">Avg Content Used</p>
                        </div>
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p
                                class="text-2xl font-extrabold {{ $summary['avg_workflow_pct'] >= 80 ? 'text-amber-600' : 'text-gray-900' }}"
                            >{{ $summary['avg_workflow_pct'] }}%</p>
                            <p class="text-[10px] text-gray-500 mt-0.5">Avg Workflow Used</p>
                        </div>
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p
                                class="text-2xl font-extrabold {{ $summary['total_over_limit'] > 0 ? 'text-red-600' : 'text-gray-900' }}"
                            >{{ $summary['total_over_limit'] }}</p>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($detailPackage['features'] as $feature)
                                    <span
                                        class="text-xs px-2.5 py-1 rounded-lg bg-[var(--brand)]/10 text-[var(--brand)] font-medium"
                                        >{{ $feature }}</span
                                    >
                                @endforeach
                            </div>
                ...
                @endphp
                    <div class="bg-blue-50 rounded-xl p-4">
                        ...
                        <div class="flex items-center gap-4 text-xs text-blue-700">
                            <span
                                ><i class="fas fa-calendar-day mr-1"></i>{{ $daysPassed }}/{{ $daysInMonth }} days
                                passed</span
                            >
                                    ...
                                    >
                                            ...
                                            <div>
                                                <div class="flex items-center justify-center gap-1">
                                                    <div class="w-12 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                                        <div
                                                            class="h-full rounded-full {{ $client['content_status'] === 'over' ? 'bg-red-500' : ($client['content_status'] === 'warning' ? 'bg-amber-500' : 'bg-green-500') }}"
                                                            style="width: {{ $client['content_pct'] }}%"
                                                        ></div>
                                                    </div>
                                                    <span
                                                        class="font-semibold {{ $client['content_status'] === 'over' ? 'text-red-600' : ($client['content_status'] === 'warning' ? 'text-amber-600' : 'text-gray-600') }}"
                                                        >{{ $client['content_used'] }}/{{ $client['content_limit'] }}</span
                                                    >
                                                </div>
                                        ...
                                        <td class="text-center">
                                            <div>
                                                <div class="flex items-center justify-center gap-1">
                                                    <div class="w-12 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                                        <div
                                                            class="h-full rounded-full {{ $client['workflow_status'] === 'over' ? 'bg-red-500' : ($client['workflow_status'] === 'warning' ? 'bg-amber-500' : 'bg-green-500') }}"
                                                            style="width: {{ $client['workflow_pct'] }}%"
                                                        ></div>
                                                    </div>
                                                    <span
                                                        class="font-semibold {{ $client['workflow_status'] === 'over' ? 'text-red-600' : ($client['workflow_status'] === 'warning' ? 'text-amber-600' : 'text-gray-600') }}"
                                                        >{{ $client['workflow_used'] }}/{{ $client['workflow_limit'] }}</span
                                                    >
                                                </div>
                                        ...
                                        <td class="text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <div class="w-12 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                                    <div
                                                        class="h-full rounded-full {{ $client['storage_status'] === 'over' ? 'bg-red-500' : ($client['storage_status'] === 'warning' ? 'bg-amber-500' : 'bg-green-500') }}"
                                                        style="width: {{ $client['storage_pct'] }}%"
                                                    ></div>
                                                </div>
                                                <span
                                                    class="font-semibold {{ $client['storage_status'] === 'over' ? 'text-red-600' : ($client['storage_status'] === 'warning' ? 'text-amber-600' : 'text-gray-600') }}"
                                                    >{{ $client['storage_used'] }}/{{ $client['storage_limit'] }}MB</span
                                                >
                                            </div>
                                        </td>
                                            ...
                                            @if ($client['contract_days_left'] !== null)
                                                @if ($client['contract_days_left'] <= 30)
                                                    <span class="text-red-600 font-semibold"
                                                        >{{ $client['contract_days_left'] }}d</span
                                                    >
                                                @else
                                                    <span class="text-gray-600"
                                                        >{{ $client['contract_days_left'] }}d</span
                ...
                <div class="p-6 space-y-5">
                        ...
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            @if ($clientDetail['priority_support'])
                                <span
                                    ...
                                    ><i class="fas fa-headset mr-0.5"></i>Priority Support</span
                                >
                            @endif
                        </div>
                        <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="bg-blue-50 rounded-xl p-4">
                        ...
                        <div class="h-2 bg-blue-200 rounded-full overflow-hidden">
                            <div
                                ...
                                style="width: {{ $clientDetail['month_progress'] }}%"
                            ></div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="rounded-xl border border-gray-100 p-4">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-gray-700"
                                    ><i class="fas fa-file-alt mr-1 text-[var(--brand)]"></i>Content</span
                                >
                                <span
                                    class="text-sm font-extrabold {{ $clientDetail['content_pct'] >= 90 ? 'text-red-600' : ($clientDetail['content_pct'] >= 70 ? 'text-amber-600' : 'text-gray-900') }}"
                                    >{{ $clientDetail['content_pct'] }}%</span
                                >
                            </div>
                            ...
                            <div class="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                    class="h-full rounded-full {{ $clientDetail['content_pct'] >= 90 ? 'bg-red-500' : ($clientDetail['content_pct'] >= 70 ? 'bg-amber-500' : 'bg-green-500') }}"
                                    style="width: {{ $clientDetail['content_pct'] }}%"
                                ></div>
                            </div>
                            <div class="flex justify-between mt-2 text-[10px] text-gray-500">
                                <span
                                    >{{ $clientDetail['content_limit'] - $clientDetail['content_used'] }} remaining</span
                                >
                        ...
                        <div class="rounded-xl border border-gray-100 p-4">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-gray-700"
                                    ><i class="fas fa-columns mr-1 text-purple-500"></i>Workflow</span
                                >
                                <span
                                    class="text-sm font-extrabold {{ $clientDetail['workflow_pct'] >= 90 ? 'text-red-600' : ($clientDetail['workflow_pct'] >= 70 ? 'text-amber-600' : 'text-gray-900') }}"
                                    >{{ $clientDetail['workflow_pct'] }}%</span
                                >
                            </div>
                            ...
                            <div class="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                    class="h-full rounded-full {{ $clientDetail['workflow_pct'] >= 90 ? 'bg-red-500' : ($clientDetail['workflow_pct'] >= 70 ? 'bg-amber-500' : 'bg-green-500') }}"
                                    style="width: {{ $clientDetail['workflow_pct'] }}%"
                                ></div>
                            </div>
                            <div class="flex justify-between mt-2 text-[10px] text-gray-500">
                                <span
                                    >{{ $clientDetail['workflow_limit'] - $clientDetail['workflow_used'] }} remaining</span
                                >
                        ...
                        <div class="rounded-xl border border-gray-100 p-4">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-gray-700"
                                    ><i class="fas fa-hdd mr-1 text-blue-500"></i>Storage</span
                                >
                                <span
                                    class="text-sm font-extrabold {{ $clientDetail['storage_pct'] >= 90 ? 'text-red-600' : ($clientDetail['storage_pct'] >= 70 ? 'text-amber-600' : 'text-gray-900') }}"
                                    >{{ $clientDetail['storage_pct'] }}%</span
                                >
                            </div>
                            ...
                            <div class="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                    class="h-full rounded-full {{ $clientDetail['storage_pct'] >= 90 ? 'bg-red-500' : ($clientDetail['storage_pct'] >= 70 ? 'bg-amber-500' : 'bg-green-500') }}"
                                    style="width: {{ $clientDetail['storage_pct'] }}%"
                                ></div>
                            </div>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <span class="text-xs text-gray-600"
                                        ><i class="fas fa-check-double mr-1 text-green-500"></i>Approvals Used</span
                                    >
                                    <span
                                        class="text-sm font-extrabold text-gray-900"
                                        >{{ $clientDetail['approvals_used'] }}</span
                                    >
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-xs text-gray-600"
                                        ><i class="fas fa-file-upload mr-1 text-orange-500"></i>Files Uploaded</span
                                    >
                                    <span
                                        class="text-sm font-extrabold text-gray-900"
                                        >{{ $clientDetail['files_uploaded'] }}</span
                                    >
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-xs text-gray-600"
                                        ><i class="fas fa-redo mr-1 text-gray-400"></i>Revisions Limit</span
                                    >
                                    <span
                                        class="text-sm font-extrabold text-gray-900"
                                        >{{ $clientDetail['revision_limit'] }}</span
                                    >
                                </div>
                            </div>
                        </div>
